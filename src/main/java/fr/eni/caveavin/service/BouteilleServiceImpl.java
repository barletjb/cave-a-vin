...
import fr.eni.caveavin.dao.CouleurRepository;
import fr.eni.caveavin.dao.RegionRepository;
...
public class BouteilleServiceImpl implements BouteilleService {
    ...
    public void supprimer(int idBouteille) {
        ...
        bRepository.findById(idBouteille)
                .orElseThrow( () -> new RuntimeException("Identifiant n'existe pas"));
    ...
    private void validerNomUnique(Bouteille bouteille){
        final Bouteille bDB = bRepository.findByNom((bouteille.getNom()));

        if (bDB != null && (bouteille.getId() == null || // Cas de la création
                !bDB.getId().equals(bouteille.getId()))) { // Cas de la modif
